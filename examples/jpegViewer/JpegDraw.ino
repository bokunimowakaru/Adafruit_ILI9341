...
void jpegDraw(File file){
    ...
    if(JpegDec.width >= JpegDec.height){
        div_x = _jpeg_div(JpegDec.width / tft.width(), JpegDec.MCUWidth);
    }else{
        axis=1;
        div_x = _jpeg_div(JpegDec.height / tft.width(), JpegDec.MCUHeight);
    }
    div_y=div_x;
    ...
    while(JpegDec.read()){
        ...
        for(by=0; by<JpegDec.MCUHeight; by+= div_y){
            for(bx=0; bx<JpegDec.MCUWidth; bx+= div_x){
                ...
                color_y=by/div_y; if( color_y > 15) color_y = 15;
                color_x=bx/div_x; if( color_x > 15) color_x = 15;
                if(color_n[color_y][color_x]) color_out= tft.color565(color[color_y][color_x][0],color[color_y][color_x][1],color[color_y][color_x][2]);
                else color_out = 0;
                /*
                if(axis) tft.goTo(y, x); else tft.goTo(x, y);
                tft.pushColor(color_out);
                */
                if(axis) tft.drawPixel(y, x,color_out);
                else tft.drawPixel(x, y,color_out);
            }
        }
    }
}
...
int jpegDrawSlide(File file){
    if(!file.isDirectory()){
        ...
        if( filename.endsWith(".jpg") || filename.endsWith(".JPG") ){
            ...
            tft.setTextSize(0);
            tft.setCursor(1, 1);
            tft.println(filename.substring(1));
            tft.setCursor(96-6*4, 1);
            tft.println("JPEG");
